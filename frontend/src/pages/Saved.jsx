...
const Saved = () => {
  ...
  const handleDelete = async (memeId) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/user/memes/saved/${memeId}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
          {savedMemes.map((meme) => (
            ...
            >
                <X color="red" size={48} className="absolute" onClick={() => handleDelete(meme.id)}/>
              <img
                src={meme.url}
                alt={meme.title}
